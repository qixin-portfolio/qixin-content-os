...
const valid = (key: "record" | "perspective" | "concise", body: string): RawCreateDraft => ({ key, body, usedFacts: [{ claim: body, factIds: ["F1"] }], interpretations: [] });
...
describe("targeted grounded-draft repair", () => {
  ...
  it("repairs only one failed draft and preserves valid bodies", async () => {
    ...
    const p = provider([record, { key: "perspective", body: "在西湖边手酸了。", usedFacts: [], interpretations: [] }, concise], { perspective: valid("perspective", "最后一直抱着他。") });
    const result = await run(p);
    expect(p.repairInputs).toHaveLength(1); expect(p.repairInputs[0].key).toBe("perspective"); expect(p.repairInputs[0].factLedger.facts.map((fact) => fact.id)).toEqual(["F1", "F2"]); expect(result.retryCount).toBe(1); expect(result.drafts.find((draft) => draft.key === "record")?.body).toBe(record.body); expect(result.drafts.find((draft) => draft.key === "record")?.key).toBe("record"); expect(result.drafts.find((draft) => draft.key === "perspective")?.qualityStatus).toBe("repaired");
  });
  it("repairs two failed drafts without sending valid drafts", async () => {
    const p = provider([valid("record", sourceText), { key: "perspective", body: "今天在公园。", usedFacts: [], interpretations: [] }, { key: "concise", body: "手酸。", usedFacts: [], interpretations: [] }], { perspective: valid("perspective", "最后一直抱着他。"), concise: valid("concise", "一张也没拍。") });
    const result = await run(p);
    expect(p.repairInputs).toHaveLength(2); expect(p.repairInputs.map((input) => input.key)).toEqual(["perspective", "concise"]); expect(p.repairInputs[0].factLedger.facts[0].text).toBe(sourceText.replace(/[。！？!?]+$/u, "")); expect(result.drafts).toHaveLength(3);
  });
  it("hides a second failed or provider-failed draft while keeping valid drafts", async () => {
    const p = provider([valid("record", sourceText), { key: "perspective", body: "在西湖边手酸。", usedFacts: [], interpretations: [] }, valid("concise", "一张也没拍。")], { perspective: new Error("provider failed") });
    const result = await run(p);
  ...
  it("sends repair only the grounding contract, never candidate text, voices, IDs, or internal titles", async () => {
    const p = provider([
      ...
      { key: "perspective", body: "在西湖边手酸。", usedFacts: [], interpretations: [] },
      valid("concise", "一张也没拍。"),
    ...
    expect(p.repairInputs[0]).toEqual({
      factLedger: expect.objectContaining({ facts: expect.any(Array) }),
    ...
    expect(JSON.stringify(p.repairInputs[0])).not.toMatch(/候选正文|VoiceSample|内部标题|databaseId|draftId|sourceItemId/i);
    expect(Object.keys(p.repairInputs[0]).sort()).toEqual(["allowedFactIds", "detailMode", "factLedger", "key", "rejectedReasons", "topic"]);
  });
  ...
  it("tries each rejected type at most once and never falls back", async () => {
    const p = provider([
      ...
      { key: "perspective", body: "在西湖边手酸。", usedFacts: [], interpretations: [] },
      valid("concise", "一张也没拍。"),
    ], { perspective: { key: "perspective", body: "在公园手酸。", usedFacts: [], interpretations: [] } });
